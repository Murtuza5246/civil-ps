...
import classes from "./writingModal.module.css";
...
export default function WritingModal(props) {
  ...
  const submitQuestionHandler = (data, mentions) => {
    ...
    setLoading(true);
    axios
      .post("/learnerPost/post", formData, options)
      .then((result) => {
        setLoading(false);
        setSelectedImagePre([]);
    ...
    <div>
      {(level >= 1 ||
        ...
        <div className={classes1.writingBoxDiv} onClick={handleClickOpen}>
          <p>
            What have you learned?.{fName ? " " + fName + " " + lName : null}
          </p>
        </div>
      )}
        ...
        <DialogContent dividers>
          {fName && (
            <div
              ...
              className={classes.writingModalName}
            >
              <span>{"@" + fName}</span>
              ...
              <input
                type="text"
                className={classes.inputClassInWriting}
                placeholder="e.g internship"
                value={doing}
                onChange={changeDoing}
              />
              <div style={{ width: "10px" }}></div>
              <strong> {" " + "at"}</strong>
              ...
              <input
                type="text"
                className={classes.inputClassInWriting}
                placeholder="problemspotter.com"
                value={At}
                onChange={changeAt}
              />
            </div>
