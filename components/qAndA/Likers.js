...
import classes from "./likers.module.css";
...
export default function CustomizedDialogs(props) {
  ...
  const handleClickOpen = () => {
    window.history.pushState(null, null, "/qanda?raisers=true");
    setOpen(true);
  };
  const handleClose = () => {
                      ...
                      <strong>
                        <Link href={"/user/details/" + items.userId}>
                          <a className={classes.nameTagHandler}>
                            {" "}
                            {items.name && items.name.substr(0, 18)}
                            {items.name && items.name.length > 18 && "..."}
                          </a>
                        </Link>
                      </strong>
